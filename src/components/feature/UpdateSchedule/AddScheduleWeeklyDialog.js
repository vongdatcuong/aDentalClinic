...
const AddScheduleWeekly = ({
    open, schedule, dates, startDate, endDate, days,
  ...
  const handleOnApply = (evt) => {
    ...
    const data = {
      start_date: fromDate,
      end_date: toDate,
      mode: lists.schedule.mode.weekly,
      value: tempDate.join(","),
    }
    // Add Schedule
    if (!scheduleID){
  ...
  return (
    <Paper>
      <Dialog
        ...
        aria-labelledby="add-schedule-weekly"
        classes={{ 
          paper: classes.paper,
        }}
      >
        <DialogTitle id="add-schedule-weekly" className={classes.dialogTitle}>{t(strings.weekly)}</DialogTitle>
        <DialogContent>
          ...
          <DialogContentText>
            <div className={classes.selectContainer}>
                ...
                >
                    {days.map((day, index) => (
                        <MenuItem key={index} value={index} style={{fontWeight: selectedDate.indexOf(index) == -1 ? 400 : 600}}>
                            {day}
                        </MenuItem>
                    ))}
                </Select>
